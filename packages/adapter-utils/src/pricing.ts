...
const OPENAI_VARIANT_RE = /^(gpt-5(?:\.\d+)?(?:-codex)?)(?:-(?:low|high|xhigh|fast|preview))+$/;
...
function stripProviderPrefix(rawModel: string): string {
  ...
  if (slash < 0) return rawModel;
  return rawModel.slice(slash + 1);
}
...
export function findKnownModelPricing(rawModel: string | null | undefined): ModelPricingEntry | null {
  ...
  const openAiVariant = unscoped.match(OPENAI_VARIANT_RE);
  if (openAiVariant) {
    return EXACT_MODEL_PRICING.get(openAiVariant[1]) ?? null;
  }

  if (unscoped.endsWith("-thinking")) {
    return EXACT_MODEL_PRICING.get(unscoped.slice(0, -"-thinking".length)) ?? null;
  }
...
export function estimateUsageCostUsd(input: {
  ...
  const cachedInputTokens = Number.isFinite(usage.cachedInputTokens ?? 0) ? (usage.cachedInputTokens ?? 0) : 0;
